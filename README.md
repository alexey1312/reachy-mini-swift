...
[git-cliff](https://git-cliff.org) from conventional commits. App Store builds, every macOS build and the notarized
macOS zip are produced locally — signing credentials never enter the repository or CI; iOS also builds continuously
on Xcode Cloud, where signing is managed by Apple and no certificate leaves the account. Every iOS and macOS build
goes to the public TestFlight group linked above; a tagged one also goes to App Review, carrying the store copy from
